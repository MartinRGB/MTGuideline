...
###
  IS ONE NUMBER WITHIN A RANGE OF ANOTHER?

  Determine whether a first value is within a range of a second value.

  Example:
  We want to animate a card out of frame if it is more than 200 pixels away from its initial x value.

  `if !Framer.Shortcuts.withinRange(myLayer.x, myLayer.originalFrame.x, 200) then ...`
###
Framer.Shortcuts.withinRange = (value, compare, range) ->
  if Math.abs(value-compare) > range 
    false 
  else 
    true
...
Layer::animateTo = (properties, first, second, third) ->
  ...
  console.log properties, first, second, third
  ...
  thisLayer.animationTo = new Animation
    ...
    curve: curve
    time: time
  console.log thisLayer.animationTo
...
###
  LAYER.DRAGGABLE HELPERS

  .constrainX() and .constrainY() are shorthand for updating the x/y value to the target layer's original x/y value on DragMove.
###
Layer::constrainDragX = ->
  @.on Events.DragMove, (evt) ->
    window.requestAnimationFrame () => @.x = @.originalFrame.x

Layer::constrainDragY = ->
  @.on Events.DragMove, (evt) ->
    window.requestAnimationFrame () => @.y = @.originalFrame.y
...
###
  ...
  Shorthand to get the x/y value of a touch event no matter the device.

  Example:
  ```myLayer.on Event.TouchEnd, (evt) ->
    console.log evt.touchX(), evt.touchY()      // log the x/y coordinates of the touch when released
  ```
###
Event::touchX = () ->
  Events.touchEvent(@).pageX

Event::touchY = () ->
  Events.touchEvent(@).pageY
